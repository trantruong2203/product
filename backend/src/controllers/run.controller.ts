...
import db, { runs, prompts, aiEngines, projects, responses, citations } from '../db/index.js';
...
export const triggerRun = async (
  ...
  try {
    ...
    if (promptIds && promptIds.length > 0) {
      promptsList = await db.select({
        id: prompts.id,
        query: prompts.query,
        projectId: prompts.projectId,
      }).from(prompts)
        .innerJoin(projects, eq(prompts.projectId, projects.id))
        .where(and(eq(prompts.isActive, true), eq(projects.userId, userId), inArray(prompts.id, promptIds)));
    } else {
      promptsList = await db.select({
        id: prompts.id,
        query: prompts.query,
    ...
    if (engineIds && engineIds.length > 0) {
      engines = await db.select().from(aiEngines)
        .where(and(eq(aiEngines.isActive, true), inArray(aiEngines.id, engineIds)));
    } else {
      engines = await db.select().from(aiEngines).where(eq(aiEngines.isActive, true));
    }
...
export const getRun = async (
  ...
  try {
    ...
    const runData = await db.select().from(runs)
    ...
    const promptData = await db.select().from(promptsAlias)
      .innerJoin(projectsAlias, eq(promptsAlias.projectId, projectsAlias.id))
    ...
    const engineData = await db.select().from(aiEngines).where(eq(aiEngines.id, run.engineId as any));
    const engine = engineData[0];

    const responseData = await db.select().from(responses).where(eq(responses.runId, run.id));

    const responseIds = responseData.map(r => r.id);
    const citationsList = responseIds.length > 0 
      ? await db.select().from(citations).where(inArray(citations.responseId, responseIds))
      : [];
